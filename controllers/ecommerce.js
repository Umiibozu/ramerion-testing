...
controller.daftarBarang = async (req, res) => { 
    ...
    const daftar = await model.produk.findAll();

    res.render("e-commerce/daftarBarang", { daftar, blogActive: "", forumActive: "", ecommerceActive:"active" });
    // res.send(daftar)
}
...
controller.detailProduk= async (req, res) => { 
  ...
  const produk = await model.produk.findOne({where:{id}});


  
  res.render("e-commerce/detailProduk", { produk, blogActive: "", forumActive: "", ecommerceActive:"active" });
}
...
controller.getAllProduk = async (req, res) => {
    try {
      await model.produk
        .findAll({
          attributes: ['id','user_id','id_kategori','nama','gambar','deskripsi','rate','harga','stok'],
          // group: ['id_kategori']
          // raw: true,
        })
        .then((result) => {
...
controller.addProduk = async (req, res) => {
    try {
        const { user_id,id_kategori,nama,deskripsi,rate,harga,stok} = req.body;
      ...
      // })
          res.redirect("/ecommerce/daftarBarang");
      } catch (error) {
...
controller.updateProduk = async (req, res) => {
    try {
      const {  user_id,id_kategori,konten,nama,gambar,deskripsi,rate,harga,stok} = req.body;
